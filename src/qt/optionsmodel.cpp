...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", KatzCoinUnits::KATZ);
    ...
    if (!settings.contains("nAnonymizeKatzCoinAmount"))
        settings.setValue("nAnonymizeKatzCoinAmount", 1000);
    nAnonymizeKatzCoinAmount = settings.value("nAnonymizeKatzCoinAmount").toLongLong();
    if (settings.contains("nMNengineRounds"))
        SoftSetArg("-mnenginerounds", settings.value("nMNengineRounds").toString().toStdString());
    if (settings.contains("nAnonymizeKatzCoinAmount"))
        SoftSetArg("-anonymizeKatzCoinamount", settings.value("nAnonymizeKatzCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeKatzCoinAmount:
            return QVariant(nAnonymizeKatzCoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case MNengineRounds:
            ...
            break;
        case AnonymizeKatzCoinAmount:
            nAnonymizeKatzCoinAmount = value.toInt();
            settings.setValue("nAnonymizeKatzCoinAmount", nAnonymizeKatzCoinAmount);
            emit AnonymizeKatzCoinAmountChanged(nAnonymizeKatzCoinAmount);
            break;
        default:
            break;
        }
