...
{
    QString ret = QString("Katz-Coin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non KATZ unit input in lnReqAmount, we generate the URI with KATZ as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(KatzCoinUnits::format(KatzCoinUnits::KATZ, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
