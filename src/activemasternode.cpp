...
//
// Bootup the masternode, look for a 10,000 KATZ input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* donations are not supported in Katz-Coin.conf */
...
{
    CKatzCoinAddress address(collateralAddress);
